...
(async () => {
  ...
  const items = await page.$$('[data-csa-c-item-type="deal"]');
  ...
  for (const item of items) {
    const hrefTarget = await item.$('a');
    const href = `${await (
      await hrefTarget.getProperty('href')
    ).jsonValue()}&tag=tokkajohotsu-22`;
    ...
    const textContent = await (
      await item.getProperty('textContent')
    ).jsonValue();
    const price = priceTarget
      ? await (await priceTarget.getProperty('textContent')).jsonValue()
      : null;
    ...
    var data = {
      ...
      aside_price: asidePriceTarget
        ? await (await asidePriceTarget.getProperty('textContent')).jsonValue()
        : null,
      image: imageTarget
        ? await (await imageTarget.getProperty('src')).jsonValue()
        : null,
  ...
  try {
    ...
    fs.writeFile(
      `docs/updated.json`,
      JSON.stringify(updated_json, null, '  '),
      (err) => {
        if (err) console.log(`error!::${err}`);
      }
    );
